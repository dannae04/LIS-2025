...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 01-03-2022 a las 22:26:13
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_insertarAutor`(in _codigo_autor varchar(6), in _nombre_autor varchar(50), in _nacionalidad varchar(50) )
BEGIN
  INSERT INTO autores VALUES(_codigo_autor, _nombre_autor, _nacionalidad );
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_listarAutores`()
BEGIN
	SELECT *FROM autores;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_listarLibros`()
BEGIN
SELECT libros.codigo_libro,
       libros.nombre_libro,
       libros.existencias,
